Shader "Custom/HealthShaderReverse"
{
	Properties
	{
	...
	SubShader
	{
		...
		void vert (inout appdata_full v)
        {
            ...
            float sinX = sin(_RotationSpeed * (_Time.y)* -1);
            float cosX = cos(_RotationSpeed * (_Time.y)* -1);
            float sinY = sin(_RotationSpeed * (_Time.y)* -1);
